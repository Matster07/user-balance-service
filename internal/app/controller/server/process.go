...
func (h *Handler) Process(deliverStatus dto.DeliverStatusDTO) error {
	...
	// Проверяем, что заказ ещё не был обработан
	...
	// Присваиваем заказу новый статус
	...
	// Находим оказываемую услугу, чтобы получить резервационный счет
	service, err := h.Service.FindById(order.ServiceId)
	if err != nil {
		return errors.New(fmt.Sprintf("service %d wasn't found", order.ServiceId))
	}

	// Определяем получателя на основе статуса выполненного заказа
	switch deliverStatus.Status {
	// Успешно - получатель - счет компании
	case "COMPLETED":
		{
	...
	// Любой другой статус - получатель - владелец
	default:
		{
